...
void main() {
  test('flushBufferToCurrentTrack should save buffered points and emit update', () async {
    final repo = _FakeRepo();
    final manager = TrackManager(repo);

    final user = _TestUser(111);
    final started = await manager.startTracking(user: user);
    expect(started, isTrue);

    // create points
    for (int i = 0; i < 6; i++) {
      manager.addGpsPoint(Position(
    ...
    await manager.flushBufferToCurrentTrack();
