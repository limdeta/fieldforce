...
class RealGpsDataSource implements GpsDataSource {
  ...
  void _startGeolocatorSubscription(LocationSettings settings) {
    ...
    _geolocatorSubscription = Geolocator
        .getPositionStream(locationSettings: settings)
        .listen(
      (position) {
        if (_isActive) {
          _logger.info('📍 $_tag: Получена GPS позиция от системы: ${position.latitude}, ${position.longitude} (accuracy: ${position.accuracy}m)');
  ...
  void _startEmissionTimer() {
    _logger.info('⏰ $_tag: Запуск таймера контролируемой эмиссии (интервал: $_emissionInterval)');
    Position? _lastEmittedPosition;
    
    _emissionTimer = Timer.periodic(_emissionInterval, (timer) {
      if (!_isActive || _positionController == null || _positionController!.isClosed) {
        timer.cancel();
        return;
      }
      ...
      if (_lastBufferedPosition != null) {
        final shouldEmit = _lastEmittedPosition == null || 
        ...
        if (shouldEmit) {
          try {
            ...
            _logger.info('📤 $_tag: Эмитирована НОВАЯ позиция: ${_lastBufferedPosition!.latitude}, ${_lastBufferedPosition!.longitude} - отправляем в LocationTrackingService');
          } catch (e, st) {
  ...
  Future<bool> checkPermissions() async {
    try {
      ...
      _logger.info('🔐 $_tag: Разрешения ${hasPermission ? 'получены' : 'отклонены'}');
  ...
  Future<Position> getCurrentPosition({required LocationSettings settings}) async {
    try {
      ...
      _logger.info('✅ $_tag: Получена текущая позиция: ${position.latitude}, ${position.longitude}');
      return position;
