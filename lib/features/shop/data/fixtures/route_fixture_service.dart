...
import 'package:fieldforce/features/shop/domain/entities/route.dart';
...
class RouteFixtureService {
  ...
  await _repository.saveRoute(yesterdayRoute, employee);
    
    // Создаем сегодняшний маршрут (в работе) - для него будет создан трек
  final todayRoute = _createTodayRoute(employee);
  await _repository.saveRoute(todayRoute, employee);
  ...
  Route _createYesterdayRoute(Employee employee) {
  ...
  Route _createTodayRoute(Employee employee) {
  print('🔧 Создаем сегодняшний маршрут для сотрудника ${employee.fullName}');
  ...
  Route _createTomorrowRoute(Employee employee) {
