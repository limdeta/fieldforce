...
class WorkDayBloc extends Bloc<WorkDayEvent, WorkDayState> {
  final GetWorkDaysForUserUseCase _getWorkDaysForUser =
      GetIt.instance<GetWorkDaysForUserUseCase>();
  final LocationTrackingServiceBase _trackingService =
      GetIt.instance<LocationTrackingServiceBase>();
  ...
  Future<void> _onLoadWorkDays(
    LoadWorkDaysEvent event,
    Emitter<WorkDayState> emit,
  ) async {
    emit(WorkDayLoading());
    ...
    try {
      ...
      emit(
        WorkDayLoaded(
          workDays: _workDays,
          selectedWorkDay: _selectedWorkDay,
  ...
  void _onSelectWorkDayByDate(
    SelectWorkDayByDateEvent event,
    Emitter<WorkDayState> emit,
  ) {
    final workDay = _workDays
        .where(
          (wd) =>
  ...
  Future<void> _onStartWorkDay(
    StartWorkDayEvent event,
    Emitter<WorkDayState> emit,
  ) async {
  ...
  Future<void> _onResumeTracking(
    ResumeTrackingEvent event,
    Emitter<WorkDayState> emit,
  ) async {
    await _trackingService.resumeTracking();
  }

  void _onUpdateActiveTrack(
