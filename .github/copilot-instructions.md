...
- Старайся делать меньше "заглушек ради того что-бы работало" часто это выливается в проблемные места типа findOneImportEntityBy ?? 1 . Где вместо критической ошибки и обдумывания как должна строиться бизнес-логика мы прибегаем к заглушкам. Если делаешь заглушки то спрашивай и оставляй коментарий TODO проверить
...
** Принципы работы с фикстурами:**
- **DevFixtureOrchestrator** - единственная точка создания и оркестрации всех фикстур
- **FixtureService** модулей (ProductFixtureService, OrderFixtureService и т.д.) - только заготовки с логикой создания, НЕ создают данные самостоятельно
- **Строгий порядок**: сначала базовые данные (категории, продукты), потом зависимые (заказы, треки)
- **Никогда не дублируйте создание**: если продукты уже созданы в DevFixtureOrchestrator, OrderFixtureService НЕ должен создавать свои
- **Зависимости через интерфейсы**: FixtureService получает Repository/UseCase через DI, но сам не решает когда создавать данные
...
- **НЕ используйте эмодзи в коде и логах** — эмодзи можно использовать только в чат-коммуникациях с пользователем. В коде, комментариях, логах, документации и commit-сообщениях используйте только текст.
...
## Модуль Shop (features/shop) - ключевые принципы архитектуры

**🏗️ Структура заказов и корзины:**
- `Order` - основная сущность заказа с состоянием (draft, submitted, approved, etc.)
- `OrderLine` - строка заказа, содержит StockItem + Product? + количество + цена
- Корзина = Order в состоянии "draft" с набором OrderLine

**🛒 Принципы работы с данными:**
- **UI работает с агрегатами**: OrderLine содержит Product для удобного отображения названий, артикулов
- **Repository загружает полные данные**: метод _getOrderLines делает JOIN с Product для заполнения OrderLine.product
- **Геттеры для UI**: OrderLine.productTitle, OrderLine.productVendorCode, OrderLine.warehouseName упрощают отображение
- **Избегать лишних абстракций**: отказались от CartItem в пользу расширенного OrderLine

- StockItem связывает Product с конкретным складом и ценами
- UpdateCartItemUseCase использует StockItemRepository для получения реальных остатков и может устанавливать quantity = 0 для удаления товаров из корзины
- Фикстуры создают связанные Product+StockItem+OrderLine для реалистичного тестирования

**🔄 Паттерн работы с корзиной:**
1. GetCurrentCartUseCase возвращает Order с заполненными OrderLine (включая Product)
2. CartBloc обрабатывает Order.lines напрямую через простые геттеры
3. UI отображает OrderLine.productTitle, orderLine.warehouseName без дополнительных преобразований
